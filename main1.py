...
def train_align(decoder_model, opt_align, args, device, X, Y, Miss_vecs, proto_Num, missindex, final_batch):
    ...
    for epoch in t_progress:
        ...
        for batch_idx, (x, y, miss_vec) in enumerate(train_loader):
            ...
            proto_Noiserobust = Noise_robust_loss(
                alpha=params["noise_alpha"]
            ).to(device)

            ins_contra = Cross_inscl_loss(
                tau=params["cross_tau"],
                alpha=params["cross_alpha"],
                lambda_conf=params["cross_lambda_conf"]
            ).to(device)
            ...
            loss_list_entropy = []
            for v in range(args.V):
                # z[v]: shape [batch_size, feature_dim]
                p_v = torch.softmax(z[v], dim=1) + 1e-12  # 避免log(0)
                entropy_v = -torch.mean(torch.sum(p_v * torch.log(p_v), dim=1))
                loss_list_entropy.append(entropy_v)
            loss_joint_entropy = sum(loss_list_entropy)
            # ====== end of joint entropy ======

            """total_loss"""
            loss_total = (
                    loss_recon
                    + params["para_loss_0"] * loss_ins_cl  # <-- 修改这里
                    + params["para_loss_1"] * loss_pro_Rob  # <-- 修改这里
                    + params["lambda_entropy"] * loss_joint_entropy  # <-- 修改这里
            )
            loss_total.backward()
            opt_align.step()
...
parser.add_argument('--i_d', type=int, default=8)
...
parser.add_argument('--para_loss_0', default=1e-4, type=float)
parser.add_argument('--para_loss_1', default=1e-2, type=float)
...
params = {
    # 从 args 获取学习率和其它参数
    "lr_pre": args.lr_pre,
    "lr_align": args.lr_align,
    "para_loss_0": args.para_loss_0,
    "para_loss_1": args.para_loss_1,
    "lambda_entropy": args.lambda_entropy,
    "protorate": args.protorate,
    ...
    "cross_tau": 0.2,
    "cross_alpha": 0.6,
    "cross_lambda_conf": 3.0,

    # Noise_robust_loss 参数
    "noise_alpha": 0.7,
}
...
def main():
    ...
    decoder_model = Network(
        views=args.V,
        input_size=args.view_dims,
    ...
    Num_C = (Protonum * params["protorate"])
    c_num = Num_C // n
    proto_Num = [[] for _ in range(n)]
    for i in range(n):
        proto_Num[i] = c_num

    optimizer_pretrain = torch.optim.Adam(decoder_model.parameters(), lr=params["lr_pre"])
    fea_emb = pretrain(decoder_model, optimizer_pretrain, args, device, X_com, Y_com, X, Y)

    optimizer_align = torch.optim.Adam(decoder_model.parameters(), lr=params["lr_align"])
    ...
    # You can return or print final_result here if needed.
    return final_result
